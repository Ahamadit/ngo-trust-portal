...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $query = "SELECT * FROM `what-we-do` WHERE id = $id LIMIT 1";
...
if (isset($_POST['submit'])) {
    ...
    $upload_dir = __DIR__ . "/uploads/what_we_do/";
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $new_image = "uploads/what_we_do/" . $image;
    ...
    if (!isset($msg)) {
        if ($action == "Add") {
            $sql = "INSERT INTO `what-we-do` (image, name, description) 
                    VALUES ('$new_image', '$name', '$description')";
        } else {
            $sql = "UPDATE `what-we-do` SET image='$new_image', name='$name', description='$description' WHERE id=$id";
        }
...
<title><?php echo $action; ?> What We Do</title>
        ...
        <div class="col-lg-6 col-md-8">
            <div class="card shadow-lg">
                <div class="card-header bg-primary text-white text-center py-3">
                    <h4 class="mb-0"><?php echo $action; ?> What We Do</h4>
                </div>
...
if (isset($msg)) {
    if ($msg == "success") {
        ...
        Swal.fire({
            ...
            window.location.href = 'what_we_do.php';
        });
        </script>";
    } elseif ($msg == "db_error") {
